...
export const getAllCourses = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { search, category, page } = req.query as any;
      let limit = 0;
    // const limit = 10
      let query:any = {};
      ...
      if (search) {
        query = {
          $or: [
            { name: { $regex: search, $options: 'i' } }, // Case-insensitive search by course name
      ...
      if(page){
        ...
        const courses = await CourseModel.find(query).skip((page -1) * limit )
        .limit(limit).select(
          "-courseData.videoUrl -courseData.suggestion -courseData.questions -courseData.links"
        )
...
// get all courses --- only for admin
export const getAdminCourseById = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const {id} = req.params;
